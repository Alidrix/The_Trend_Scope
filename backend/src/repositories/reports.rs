...
pub async fn jobs_snapshot(pool: &PgPool) -> Result<serde_json::Value, AppError> {
    let pending_reports: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM reports WHERE status='pending'")
            .fetch_one(pool)
            .await?;
    let completed_24h:i64=sqlx::query_scalar("SELECT COUNT(*) FROM reports WHERE status='completed' AND completed_at>=NOW()-INTERVAL '24 hours'").fetch_one(pool).await?;
    let pending_alerts: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM alert_deliveries WHERE status='pending'")
            .fetch_one(pool)
            .await
            .unwrap_or(0);
    let sent_24h:i64=sqlx::query_scalar("SELECT COUNT(*) FROM alert_deliveries WHERE status IN ('sent','logged') AND created_at>=NOW()-INTERVAL '24 hours'").fetch_one(pool).await.unwrap_or(0);
    Ok(
        serde_json::json!({"pending_reports":pending_reports,"completed_reports_24h":completed_24h,"pending_alert_deliveries":pending_alerts,"sent_alert_deliveries_24h":sent_24h,"recent_reports":[],"recent_alert_deliveries":[]}),
    )
}
